...
$(function(){
    ...
    const pathLength=path.getTotalLength(); //경로의 총 길이를 계산
    path.style.strokeDasharray=pathLength;//경로의 점선처럼 보이게 설정
    ...
    gsap.timeline({
        scrollTrigger:{
            ...
            end:'100% 100%',//#con3의 맨아래(100%)가 화면의 맨아래(100%)에 닿을때 애니끝
            scrub:1,
            //스크롤 움직임에 따라서 애니메이션도 같이 움직이게 합니다.
        }
    }).to('#con3',{
            backgroundColor:'#fff',
            color:'#000',
    ...
    .fromTo('#con3 .videowrap video',{'clip-path':'inset(60% 60% 60% 60% round 30%)'},{'clip-path':'inset(0% 0% 0% 0% round 0%)',duration:5, ease:'none'},0)
   ...
   //각 애니메이션을 트리거하는 함수
    ...
    pageButtons.forEach((button,index)=>{
        button.addEventListener('click',()=>{
          const targetPosition=-100*index; //각 버튼의 해당하는 위치 계산
